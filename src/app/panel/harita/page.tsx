...
export default async function HaritaPage() {
  ...
  const input: FieldMapInput[] = fields.map((f) => ({
    id: f.id,
    name: f.name,
    area: f.area,
    posX: f.posX,
  ...
  const rects = layoutFields(input);
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500">
          {fields.length} tarla · kusbakisi 2D yerlesim (bir tarlaya tiklayin)
        </p>
      </div>
